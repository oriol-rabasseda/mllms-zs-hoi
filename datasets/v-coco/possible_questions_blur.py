...
for vcoco in vcoco_all:
    for i in range(len(vcoco['image_id'])):
        ...
        if obj == '' and instr == '':
            question = f"Is the foreground person {hoi['action_gerund']}? Answer yes or no."
        elif obj == '':
            question = f"Is the foreground person {hoi['action_gerund']} with {hoi['instr']}? Answer with yes or no."
        elif instr == '':
            question = f"Is the foreground person {hoi['action_gerund']} a {hoi['obj']}? Answer with yes or no."
        else:
            question = f"Is the foreground person {hoi['action_gerund']} a {hoi['obj']} with a {hoi['instr']}? Answer with yes or no."
...
with open("annotations/questions_per_hoi_blur.json", 'w') as f:
